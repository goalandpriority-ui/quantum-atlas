...
function buildIndex(): SearchResult[] {
  ...
  dictionaryTerms.forEach((t) => {
    results.push({
      ...
      category: "Dictionary",
      categoryColor: categoryColors["Dictionary"],
    });
  });
  ...
  algorithms.forEach((a) => {
    results.push({
      ...
      category: "Algorithm",
      categoryColor: categoryColors["Algorithm"],
    });
  });
  ...
  processors.forEach((p) => {
    results.push({
      slug: p.slug,
      title: p.name,
      summary: p.summary.slice(0, 140) + "...",
      href: `/hardware/${p.slug}`,
      category: "Hardware",
      categoryColor: categoryColors["Hardware"],
    });
  });
  ...
  industries.forEach((i) => {
    results.push({
      ...
      category: "Industry",
      categoryColor: categoryColors["Industry"],
    });
  });

  researchPapers.forEach((r) => {
    results.push({
      slug: r.slug,
      title: r.title,
      summary: r.plainSummary.slice(0, 140) + "...",
      href: `/research/${r.slug}`,
      category: "Research",
      categoryColor: categoryColors["Research"],
    });
  });
  ...
  learningLevels.forEach((level) => {
    level.articles.forEach((article) => {
      results.push({
        ...
        category: "Learn",
        categoryColor: categoryColors["Learn"],
      });
    });
  });
  ...
  courses.forEach((c) => {
    results.push({
      slug: c.slug,
      title: c.title,
      summary: c.description.slice(0, 140) + "...",
      href: `/courses/${c.slug}`,
      category: "Course",
      categoryColor: categoryColors["Course"],
    });
  });
    ...
    <span>
      {parts.map((part, i) =>
        part.toLowerCase() === query.toLowerCase() ? (
          <mark key={i} className="bg-quantum-50 text-quantum rounded px-0.5">
            {part}
          </mark>
        ) : (
    ...
    <div className="max-w-content mx-auto px-6 py-14">
      ...
      <div className="relative max-w-2xl mb-8">
        ...
        <input
          ...
          placeholder="Search algorithms, terms, hardware, companies…"
          className="w-full pl-11 pr-4 py-3.5 rounded-xl border border-line bg-surface text-ink placeholder:text-ink-soft focus:outline-none focus:border-quantum transition-colors text-sm"
        />
        {query && (
          <button
          ...
          {filtered.map((result) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <span className={`font-mono text-[10px] uppercase tracking-wide rounded-full px-2 py-0.5 ${result.categoryColor}`}>
                  {result.category}
                </span>
              </div>
              <h2 className="font-serif text-base font-semibold text-ink mb-1 leading-snug">
      ...
      {!query.trim() && (
        <div className="max-w-2xl">
          <p className="text-ink-muted text-sm mb-6">
            Search across {index.length}+ entries — dictionary terms,
            algorithms, hardware, companies, industries, research papers,
            learning articles, and courses.
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {["Shor's Algorithm", "Qubit", "IBM Condor", "VQE"].map((suggestion) => (
              <button
                ...
                onClick={() => {
                  setQuery(suggestion);
                  router.replace(`/search?q=${encodeURIComponent(suggestion)}`, { scroll: false });
                }}
                className="rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink-muted hover:border-quantum hover:text-quantum transition-colors text-left"
              >
                {suggestion}
              </button>
            ))}
