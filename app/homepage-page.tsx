...
export default function HomePage() {
  ...
  return (
    <div>
      ...
      {/* All Sections - full site map, organized in 3 rows */}
      <section className="max-w-content mx-auto px-6 py-14 border-b border-line">
        <p className="font-mono text-xs uppercase tracking-widest text-quantum mb-2">
          Everything On This Site
        </p>
        <h2 className="font-serif text-2xl md:text-3xl font-semibold text-ink mb-8">
          16 Sections, One Encyclopedia
        </h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mb-8">
          {[
            ...
            <Link
              key={item.href}
              href={item.href}
              className="rounded-xl border border-line bg-surface p-4 text-center hover:border-quantum hover:shadow-[0_4px_20px_-8px_rgba(52,84,209,0.35)] transition-all"
            >
              <p className="text-xl mb-1.5">{item.icon}</p>
              <p className="text-xs text-ink font-medium leading-snug">{item.label}</p>
            </Link>
          ))}
        </div>

        {/* Row 2: Insights & Analysis */}
        <p className="font-mono text-xs uppercase tracking-wide text-ink-soft mb-3">
          Insights &amp; Analysis
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mb-8">
          {[
            { href: "/quantum-vs-classical", label: "Quantum vs Classical", icon: "⚖️" },
            { href: "/quantum-vs-ai", label: "Quantum vs AI", icon: "🤖" },
            { href: "/future", label: "Future Predictions", icon: "🔮" },
            { href: "/myths", label: "Myths Debunked", icon: "💭" },
            { href: "/research", label: "Research Papers", icon: "📄" },
      ...
      {/* Quick nav strip - key stats */}
      <section className="max-w-content mx-auto px-6 py-10 border-b border-line">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          {[
            { href: "/algorithms", label: "Algorithms", count: "50" },
            { href: "/hardware", label: "Hardware", count: "14" },
            { href: "/companies", label: "Companies", count: "10" },
            { href: "/industries", label: "Industries", count: "10" },
            { href: "/research", label: "Research", count: "20" },
            { href: "/myths", label: "Myths", count: "30" },
          ].map((item) => (
            <Link
              key={item.href}
              href={item.href}
